...
type Order struct {
	*gorm.Model
	...
	TableID       *uint `json:"table_id" gorm:"type:uuid;index"` // Nullable for takeout orders
	PaymentID   *uint `json:"payment_id" gorm:"type:uuid;index"` // Nullable for unpaid orders
	TableNumber   string    `json:"table" gorm:"not null;size:50;index"`
	OpenedAt      time.Time `json:"opened_at" gorm:"not null"`
	IsClosed      bool      `json:"is_closed" gorm:"default:false;index"`
	Status        string    `json:"status" gorm:"default:open;size:50"`
